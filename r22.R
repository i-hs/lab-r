...
mydata <- read.csv(file = "mlwr/usedcars.csv",
                   stringsAsFactors = F)
...
# 그 구간에 해당하는 데이터들의 갯수(빈돗수) 막대로 그린 그래프
...
# 이산형 데이터(discrete data) 또는 범주형 데이터의 빈돗수
table(mydata$year)
table(mydata$model)
table(mydata$color)
