...
# 변수이름 <- 값
# 변수이름 = 값
x <- 11   # Ctrl+Enter: 해당 커서가 위치하는 R 문장을 실행 
y = 22
...
num_array <- 1:10  # num_array 변수에 1 ~ 10까지의 숫자들을 저장
...
# 벡터와 스칼라를 사칙연산(+, -, *, /)를 하면,
# 벡터의 모든 원소에 하나씩 스칼라 값을 사칙연산하게 됨.
...
# 함수를 호출할 때 파라미터 이름을 명시하지 않으면,
# 함수 정의의 파라미터 순서대로 값들이 전달됨.
seq2 <- seq(1, 10, 3)
seq2
# R에서는 함수를 호출할 때 파라미터 이름을 명시하면,
# 함수 정의의 파라미터 순서와 상관없이 함수를 호출할 수 있음.
seq3 <- seq(by = 3, from = 1, to = 10)
seq3
...
search()  # 검색 경로(메로리)에 로드된 패키지, 환경 정보 등을 확인
