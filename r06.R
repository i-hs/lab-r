...
# 영어점수 평균 이상인 학생들의 id, class, english를 출력
exam[exam$english >= mean(exam$english), 
     c("id", "class", "english")]
...
# mpg 데이터 프레임에서
# 시내 주행 연비가 평균 이상이고, 고속도로 연비도 평균 이상인
# 자동차들의 model, cty, hwy를 출력
mpg[mpg$cty >= mean(mpg$cty) & mpg$hwy >= mean(mpg$hwy),
    c("model", "cty", "hwy")]


# exam 데이터 프레임에서 1반 학생들의 수학 점수 평균
mean(exam$math)  # 전체 학생들의 수학 점수 평균
class1 <- exam[exam$class == 1, ]  # 1반 학생들로만 이루어지 DF
